...
void hid_task_multiplexer(void *pvParameters) {
  ...
  // Consumer loop
  while (true) {
    hid_keyboard_report_t queue_received;
    if (xQueueReceive(global_hid_report_queue, &queue_received, portMAX_DELAY) == pdTRUE) {
      ...
      #if DEBUG_LOG
      print_keyboard_report(pcTaskGetName(NULL), queue_received);
      ...
      if (tud_hid_keyboard_report(HID_ITF_PROTOCOL_KEYBOARD, queue_received.modifier, queue_received.keycode) == false && tud_ready()) {
...
void hid_init_multiplexer(){
    ...
    global_hid_report_queue = xQueueCreate(10, sizeof(hid_keyboard_report_t));
